var ChartConstantLinesExtension = (function() {
    ...
    function onItemWidgetOptionsPrepared(args)  {
        if(args.dashboardItem instanceof Model.PieItem) {
            // const piecharts = args.getWidget;
            // const piechart = piecharts[0];
            if (args.itemName === "pieDashboardItem1") {
                args.options["type"] = "doughnut";           
                args.options["palette"] = "Carmine";
            } else {
                args.options["type"] = "doughnut";
                args.options["palette"] = "Ocean";
            }

            args.options["centerTemplate"] = (pieChartItem, svgElement) => {
                const heavyFontSize = Math.floor(pieChartItem.getInnerRadius() / 2);
                const lightFontSize = Math.floor( heavyFontSize / 2 );
                var content = $('<svg>' +
                '<style>' + 
                '.heavy {font: bold ' + heavyFontSize + 'px sans-serif; }'+ 
                '.light {font: ' + lightFontSize + 'px sans-serif; }' +
                '</style>' +
                '<text x="70" y="25" text-anchor="middle" class="heavy" fill="#000">64%</text>' +
                '<text x="12" y="60" class="light" fill="#000">Total 456</text>' +  
                '</svg >');

                svgElement.appendChild(content.get(0));
            };
